...
- `_squeeze_context_results()`: кожна категорія джерел має **окремий** cap. `wiki_cap=1` (фоновий контент). `zir_cap=2` (офіційна позиція ДПС). `positions_cap=2` (правові позиції ВС — 12 800 Q&A). `court_cap=max(1,min(2,target//4))` (supreme + CCU + rada_court). `regular_col_cap=max(2,target//2)` (Rada collections + KMU + MOD — не більше половини слотів на одну колекцію; `target//3` виявилось занадто жорстким для вузькотематичних запитів). Не об'єднувати назад — кожне джерело має свою природу і cap.
