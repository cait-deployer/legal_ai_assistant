...
function CoverageBar({ pct }: { pct: number | null }) {
  if (pct == null) return <div className="h-1 rounded-full bg-[#C9A84C]/10 w-full" />
  const color = pct >= 80 ? "bg-emerald-500" : pct >= 40 ? "bg-amber-500" : "bg-red-500"
  return (
    <div className="h-1 rounded-full bg-[#C9A84C]/10 w-full overflow-hidden">
    ...
    <div className="flex flex-col gap-4 sm:gap-6 pb-8">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2 sm:p-3 bg-[#C9A84C]/10 border border-[#C9A84C]/20 rounded-xl sm:rounded-2xl shrink-0">
            <ShieldCheck className="w-5 h-5 sm:w-8 sm:h-8 text-[#C9A84C]" />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {data?.cache_age_sec != null && (
            <span className="text-[10px] font-black uppercase tracking-widest text-[#C9A84C]/40 hidden sm:block">
              Дані: {Math.round(data.cache_age_sec / 3600)} год. тому
            </span>
          )}
          <Button
          ...
          >
            ...
            <span className="hidden sm:inline">{refreshing ? "Оновлюємо…" : "Оновити з Ради"}</span>
          </Button>
        </div>
      </div>
      ...
      {loading && (
        <div className="grid gap-2 sm:gap-3">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-14 rounded-2xl bg-[#C9A84C]/5 animate-pulse" style={{ animationDelay: `${i * 50}ms` }} />
          ))}
        </div>
      )}
      ...
      {!loading && !error && data && (
        <>
          ...
          <p className="text-[10px] sm:text-xs text-[#C9A84C]/40 -mt-2">
          ...
          <div className="sm:hidden rounded-2xl border border-[#C9A84C]/15 overflow-hidden divide-y divide-[#C9A84C]/8">
            ...
            {visible.length === 0 && (
              <div className="py-10 text-center text-[#E0E6ED]/40 text-sm">
                Немає розділів з таким статусом
              </div>
            )}
          </div>

          {/* ── Sections: DESKTOP table ── */}
