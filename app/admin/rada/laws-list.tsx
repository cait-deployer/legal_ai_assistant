...
import { LawCard, CATEGORY_LABELS } from "./law-card"
import { LawTable } from "./law-table"
import { LawDetail } from "./law-detail"
...
export function LawsListTab() {
  ...
  const fetchLaws = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch("/api/admin/rada/laws?per_page=2000&page=1", {
        cache: "no-store",
      })
      if (!res.ok) throw new Error("Не вдалося завантажити закони")
  ...
  // reset page on search / per_page change
  useEffect(() => { setCurrentPage(1) }, [search, itemsPerPage])
  // close detail when search changes
        ...
        <div className="shrink-0 pt-6 pb-3 space-y-4 border-b border-border bg-background">
            ...
            {Object.entries(categoryStats).map(([cat, count]) => (
              <Badge
                ...
                className={`px-3 py-1.5 text-sm rounded-lg font-medium border ${
                  cat === "h14" ? "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/20 dark:text-blue-400 dark:border-blue-800" :
                  cat === "h25" ? "bg-red-50 text-red-700 border-red-200 dark:bg-red-950/20 dark:text-red-400 dark:border-red-800" :
                  cat === "h19" ? "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-800" :
                  cat === "h20" ? "bg-green-50 text-green-700 border-green-200 dark:bg-green-950/20 dark:text-green-400 dark:border-green-800" :
                  ""
                }`}
              >
          ...
          <div className="flex gap-3 items-center flex-wrap">
            ...
            <div className="relative flex-1 min-w-[200px] h-10">
              ...
              <Input
                placeholder="Пошук за назвою, ID, категорією, статусом…"
                type="search"
                value={search}
            ...
            <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center gap-4">
              ...
              <div>
                <p className="text-lg font-semibold">
                  {search ? "Нічого не знайдено" : "Закони відсутні"}
                </p>
