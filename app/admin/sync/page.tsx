...
import { RefreshCw, Download, HardDriveDownload, Cpu, RotateCcw, AlertCircle, Clock, Save } from "lucide-react"
...
const SCHEDULE_SOURCE_META: Record<string, { label: string }> = {
  rada:      { label: "Верховна Рада (legacy V1)" },
  kmu:       { label: "Кабінет Міністрів" },
  ccu:       { label: "Конституційний суд" },
  supreme:   { label: "Верховний суд" },
  wiki:      { label: "Legal Aid Wiki" },
  positions: { label: "Правові позиції ВС" },
  mod:       { label: "Міністерство оборони" },
  zir:       { label: "ЗІР ДПС" },
}
...
function ScheduleWidget() {
  ...
  return (
    <div className="bg-[#111827] rounded-2xl border border-[#C9A84C]/10 p-4 sm:p-5 space-y-4">
      ...
      {/* Schedule hour */}
      <div className="flex items-center gap-3">
        <span className="text-xs text-gray-400 shrink-0">Час запуску (UTC):</span>
        <select
          value={hour}
        ...
        <span className="text-[10px] text-gray-600">щодня для всіх ввімкнених джерел</span>
      </div>
      ...
      {!status ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {Object.entries(SCHEDULE_SOURCE_META).map(([src, meta]) => {
            const srcStatus = status.sources[src]
            return (
              <label key={src} className="flex items-center justify-between gap-2 bg-[#0d1120] rounded-xl px-3 py-2 cursor-pointer hover:bg-[#0f1628] transition-colors">
                <div>
                  <div className="text-xs font-medium text-[#E0E6ED]">{meta.label}</div>
                  {srcStatus?.last_sync && (
                    <div className="text-[10px] text-gray-600">
                      Останній: {new Date(srcStatus.last_sync).toLocaleString("uk-UA", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
                    </div>
                  )}
                  {!srcStatus?.last_sync && (
                    <div className="text-[10px] text-gray-700">Ще не запускалось</div>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {srcStatus?.running && (
                    <span className="text-[10px] text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded">Активний</span>
                  )}
                  <button
      ...
      <div className="text-[10px] text-gray-700 border-t border-white/5 pt-3">
        Авто-синхронізація завантажує нові документи зі скраперів. Реіндекс потрібно запускати окремо після скрапінгу.
      </div>
    </div>
  )
}
