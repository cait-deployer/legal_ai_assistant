...
function StatusBadge({ status }: { status?: string }) {
  ...
  const isActive =
    status.toLowerCase().includes('чинний') &&
    !status.toLowerCase().includes('втратив');
...
function MessageText({ text, refs, onCitationOpen }: { text: string; refs: Citation[]; onCitationOpen: (c: Citation) => void }) {
  ...
  const parseInline = (lineText: string) => {
    ...
    return parts.map((part, i) => {
      if (/^\[\d+(?:,\s*\d+)*\]$/.test(part)) {
        const nums = part.slice(1, -1).split(',').map(n => n.trim());
  ...
  lines.forEach((line, i) => {
    if (line.startsWith('### ')) nodes.push(<h3 key={i} className="font-bold text-slate-900 mt-3 mb-1 text-base tracking-tight">{parseInline(line.slice(4))}</h3>);
    else if (/^[\*\-]\s/.test(line)) nodes.push(<div key={i} className="flex gap-2 my-1 pl-1"><div className="h-1 w-1 rounded-full bg-indigo-400 mt-2 shrink-0" /> <span className="text-slate-700 text-sm leading-relaxed">{parseInline(line.slice(2))}</span></div>);
    else if (line === '') nodes.push(<div key={i} className="h-2" />);
    else nodes.push(<p key={i} className="my-1 text-sm leading-relaxed text-slate-700">{parseInline(line)}</p>);
  });
        ...
        <header className="h-16 border-b border-slate-100 flex items-center px-6 justify-between bg-white/80 backdrop-blur-md sticky top-0 z-30 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <h1 className="font-extrabold text-lg tracking-tight text-slate-900 uppercase">Legal <span className="text-indigo-600">AI</span></h1>
            </div>
          </div>
          ...
          <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
            {messages.map(msg => (
              <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`h-9 w-9 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${msg.role === 'ai' ? 'bg-slate-900 text-white' : 'bg-indigo-600 text-white'}`}>
                ...
                <div className={`group relative max-w-[85%] ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                  <Card className={`p-4 text-sm shadow-sm border-none ring-1 ring-slate-200/60 ${msg.role === 'user'
            ...
            {isLoading && (
              <div className="flex gap-3 items-center pl-1">
                ...
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Thinking...</span>
              </div>
            )}
          </div>
        </div>
        ...
        <DialogContent className="sm:max-w-md bg-white rounded-3xl p-6 border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3 text-slate-900 text-lg font-bold uppercase tracking-tight">
              <div className={`${launchTool?.color} ${launchTool?.bg} p-2 rounded-lg`}>
          ...
          <div className="my-4">
            <p className="text-xs text-slate-500 leading-relaxed font-medium mb-4">{launchTool?.description}</p>
            {launchTool?.inputLabel && (
              <div className="space-y-2">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{launchTool.inputLabel}</label>
                <Textarea
          ...
          <DialogFooter>
            <Button onClick={handleLaunchConfirm} className="bg-slate-900 hover:bg-black h-11 w-full rounded-xl font-bold text-xs tracking-wide text-white transition-all">
              GENERATE ARTIFACT
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-4xl bg-white rounded-3xl p-6 border-none shadow-2xl">
          <DialogHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4 mb-4">
            <DialogTitle className="text-slate-900 font-bold uppercase tracking-tight text-lg">
              <Zap className="inline h-5 w-5 text-indigo-600 mr-2" /> {toolResult?.title}
            </DialogTitle>
          </DialogHeader>
          {toolResult && (
          ...
          <ScrollArea className="max-h-[360px] p-8 bg-white text-sm">
            <div className="space-y-6">
              {activeCitation?.passages.map((p, i) => (
                <div key={i} className="relative pl-4 border-l-2 border-indigo-100 text-slate-800 leading-relaxed">
                  &ldquo;{p}&rdquo;
                </div>
              ))}
            </div>
