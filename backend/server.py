        ...
        async def _rewrite_query(q: str) -> str | None:
            ...
            try:
                ...
                resp = await _asyncio.to_thread(
                    _m.generate_content,
                    (
                        f"як платять відрядні за кордон → норми відшкодування витрат на відрядження за кордон\n"
                        f"скільки днів відпустки на рік → тривалість щорічної основної оплачуваної відпустки\n"
                        f"як звільнити працівника без його згоди → підстави розірвання трудового договору з ініціативи роботодавця\n"
                        f"які добові при відрядженні за кордон → суми та склад витрат на відрядження за кордон норми відшкодування добових витрат\n"
                        f"добові за кордон скільки → граничні норми добових витрат службове відрядження за кордон постанова КМУ\n"
                        f"які суми добових для держслужбовців → норми добових витрат державних службовців при відрядженні\n"
                        f"скільки платять добових держслужбовцям за кордон → суми відшкодування добових витрат державних службовців у закордонних відрядженнях\n"
                        f"скільки отримують держслужбовці за день у відрядженні → розмір добових державних службовців при відрядженні за кордон\n"
                        f"яка добова ставка для чиновників → норми добових для державних службовців згідно постанови КМУ\n"
                        f"скільки відсотків ПДВ → ставка податку на додану вартість розмір відсоткова ставка ПДВ\n"
                        f"штраф за прострочення договору → відповідальність за порушення строків виконання договірних зобов'язань неустойка пеня\n"
                        f"{q} →"
                    ),
    ...
    try:
        ...
        if low_confidence:
            response_instructions.append(
                ...
                "3) Окремим абзацом: «⚠️ Точної норми щодо [аспект запиту] не знайдено. "
            ...
            response_instructions.append(
                ...
                "3) Якщо конкретної норми (суми, строки, відсотки) немає в наданому тексті — "
