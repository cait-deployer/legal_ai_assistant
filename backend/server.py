...
def _route_collections(question: str, all_collections: list) -> list[str]:
    """
    Визначає релевантні колекції Qdrant для конкретного питання без LLM-виклику.

    Завжди включаємо:
      laws_positions — відформульовані правові позиції ВС (найвища якість)
      laws_supreme   — повні рішення Верховного Суду

    Опційно — за правовим доменом:
      rada_finance   — фінанси, податки, ФОП, митниця
      rada_labor     — трудове, мобілізація, ЗСУ
      rada_criminal  — кримінальне
      rada_civil     — цивільне, сімейне, спадщина
      rada_industry  — господарське, корпоративне, банкрутство
      rada_admin     — адміністративне, ліцензування, держоргани
      rada_housing   — нерухомість, ЖКГ, будівництво
      rada_land      — земельне, кадастр, агро
      rada_state     — держустрій, громадянство
      rada_court     — судовий процес, виконавче провадження
      rada_intl      — міжнародне, ЄСПЛ
      laws_ccu       — конституційне право, права людини
      laws_wiki      — визначення понять (рідко)

    Якщо жоден домен не розпізнано → ALL_COLLECTIONS (безпечний fallback).
    """
    q = question.lower()
    ...
    # "податок" → "податку", "податком", "податкова", "податкові" тощо.
    RULES: list[tuple[list[str], list[str]]] = [

        # Фінансове / податкове / митне право
        (
            [
                "пдв", "податк", "оподат", "єдин податок", "спрощен систем",
                "єсв", "акциз", "митн", "фіскал", "бухгалтер",
                "декларац", "платник податк", "ндфл", "ппр",
                "донараху", "камеральн", "уточнюючий розрахунок",
                "податкова перевірк", "фоп", "ставк пдв",
                "розрахункова операц", "рро",
            ],
            ["rada_finance"],
        ),

        # Трудове право / мобілізація / ЗСУ
        (
            [
                "трудов", "звільнен", "зарплат", "відпуст",
                "лікарнян", "праців", "роботодав",
                "колективн договір", "страйк",
                "мобіліз", "військовозобов", "бойов",
                "зсу", "ато", "ухилення від призов",
                "контракт військов", "дембель",
            ],
            ["rada_labor"],
        ),
        ...
        (
            [
                "злочин", "кримінальн", "обвинувач", "вирок",
                "засуджен", "арешт", "набу", "детектив сап",
                "прокурор", "підозрюван", "кримінальне провадж",
                "корупц", "хабар", "шахрайств", "вбивств",
                "крадіжк", "розбій", "грабіж", "наркотик",
            ],
            ["rada_criminal"],
        ),
        ...
        (
            [
                "договір купівл", "договір оренд", "спадщин",
                "шлюб", "розлучен", "аліменти", "цивільн відпов",
                "відшкодуван шкод", "борг", "кредит договір",
                "позик", "дарув", "заповіт",
                "усиновл", "опік", "батьківськ права",
                "розірвання договор",
            ],
            ["rada_civil"],
        ),
        ...
        (
            [
                " тов ", "акціонер", "банкрутств", "корпоратив",
                "господарськ", "засновник", "статутн капітал",
                "реєстрац підприємств", "ліквідац підприємств",
                "санація", "кредитор вимог", "акціонерн товариств",
            ],
            ["rada_industry"],
        ),

        # Адміністративне право / ліцензування / держоргани
        (
            [
                "ліцензі", "дозвіл на ", "адміністратив відпов",
                "адмін штраф", "держорган", "держслужб",
                "нотаріат", "перевірк дпс", "перевірк держ",
                "патентн", "сертифікац", "оскаржен рішен органу",
                "адміністративн суд",
            ],
            ["rada_admin"],
        ),

        # Нерухомість / будівництво / ЖКГ
        (
            [
                "нерухомість", "квартир", "будинок", "іпотек",
                "оренд нерух", "жкг", "комунальн послуг",
                "будівництв", "реєстрац прав нерух",
                "управитель будинк",
            ],
            ["rada_housing"],
        ),
        ...
        (
            [
                "земел", "земельн ділянк", "кадастр",
                "земельний пай", "сільськогосподарськ",
                "оренд землі", "цільов призначен",
                "рнсхп", "агропромислов",
            ],
            ["rada_land"],
        ),

        # Держустрій / громадянство / адмін реєстрація
        (
            [
                "громадянств", "паспорт", "реєстрац місця проживан",
                "держустрій", "публічн служб", "орган місцев",
                "вибор", "референдум", "громад організац",
            ],
            ["rada_state"],
        ),

        # Судовий процес / виконавче провадження
        (
            [
                "апеляц", "касац скарг", "виконавч провадж",
                "виконавч лист", "судов витрат", "процесуальн строк",
                "забезпечен позову", "мирова угод",
                "підвідомственість", "підсудність",
            ],
            ["rada_court"],
        ),

        # Міжнародне право / ЄСПЛ
        (
            [
                "єспл", "міжнародн конвенц", "ратифікац",
                "міжнародн договір", "страсбург",
                "конвенція про захист прав",
            ],
            ["rada_intl"],
        ),

        # Конституційне право / права людини
        (
            [
                "конституцій", "права людин", "основн права і свобод",
                "дискримінац", "конституційний суд", " кцу ",
                "конституційність",
            ],
            ["laws_ccu", "rada_state"],
        ),
    ...
    matched = False
    for keywords, collections in RULES:
        if any(kw in q for kw in keywords):
            selected.update(collections)
            matched = True

    # Визначення понять → додаємо вікі
    if any(kw in q for kw in ["що таке ", "як визначаєтьс", "поняття ", "визначення поняття"]):
        selected.add("laws_wiki")

    # Нічого не розпізнано → безпечний fallback: шукаємо скрізь
    if not matched:
        return list(all_collections)

    routed = sorted(selected)  # стабільний порядок для логів
    print(f"🗺️  [Router] {len(routed)} колекцій: {routed}")
    return routed
...
async def ask(body: AskRequest):
    ...
    if body.filter_sources:
        ...
        target_collections = _route_collections(question, ALL_COLLECTIONS)
