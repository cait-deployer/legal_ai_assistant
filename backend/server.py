...
async def ask(body: AskRequest):
    ...
    try:
        ...
        # Retrieval quality guardrail — захист від галюцинацій при слабкому пошуку
        _retrieval_top = results[0]["similarity"] if results else 0.0
        if _retrieval_top < 0.68:
            response_instructions.append(
                "ВАЖЛИВО: знайдені документи можуть бути лише частково релевантними до питання. "
                "Якщо серед джерел НЕМАЄ прямої норми, яка відповідає на питання, — "
                "скажи чітко: «У наявних джерелах точної норми з цього питання не знайдено» "
                "і вкажи куди звернутись (наприклад: офіційний реєстр, ДІАМ, профільне міністерство). "
                "НЕ вигадуй зв'язок між документом і питанням, якщо його немає."
            )
