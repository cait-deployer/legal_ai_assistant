    ...
    try:
        ...
        if _retrieval_top < 0.68:
            response_instructions.append(
                ...
                "2) Окремим абзацом в кінці: «Точної норми щодо [конкретний аспект запиту] "
                "в наявних джерелах не знайдено. Рекомендую звернутись до: [конкретне місце].» "
                "НЕ починай відповідь з «не знайдено» — спочатку покажи що є."
            )

        # Clarifying question — якщо питання неоднозначне, краще уточнити
        response_instructions.append(
            "Якщо питання неоднозначне і відповідь суттєво залежить від деталей "
            "(наприклад: організація бюджетна чи приватна, особа — держслужбовець чи ні, "
            "конкретна країна відрядження, тип договору, регіон тощо) — "
            "задай ОДНЕ конкретне уточнювальне питання замість неповної або узагальненої відповіді. "
            "Питання формулюй коротко і конкретно. Не задавай кілька питань одночасно."
        )
