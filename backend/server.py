...
async def _ask_pipeline(body: AskRequest) -> dict:
    ...
    try:
        ...
        response_instructions.append(
            "КОМПАКТНІ ДЖЕРЕЛА: не додавай довгі уривки, URL або повний список усіх знайдених джерел у текст відповіді. "
            "Достатньо цитат [N] після тверджень. Якщо потрібен блок джерел, дай максимум 2-4 найрелевантніші джерела "
            "у форматі: [N] коротка назва — що підтверджує. Не переказуй кожен знайдений документ."
        )
        response_instructions.append(
        ...
        if low_confidence and is_short_response:
            ...
            response_instructions.append(
                ...
                "Структура відповіді: "
                "1) коротко скажи, що прямої відповіді немає або вона часткова; "
                "2) назви максимум 2-3 найближчі релевантні джерела з [N] і що саме вони підтверджують; "
                "3) дай один безпечний наступний крок або одне уточнювальне питання. "
            ...
            response_instructions.append(
                ...
                "Структура відповіді: "
                "1) короткий висновок; "
                "2) що прямо підтверджено 1-3 найрелевантнішими джерелами; "
                "3) чого бракує для точної відповіді; "
                "4) один наступний безпечний крок. "
        ...
        if is_short_response:
            ...
            _pref_limits = {"standard": 280, "detailed": 650, "full": 1100}
